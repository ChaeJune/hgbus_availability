...
- 가동률(%)은 status page 관례를 따라 **전체 운항 예정 시간 대비 기록된 결항 시간**으로
  계산합니다(기록이 없는 기간은 정상으로 간주, 일부 결항은 50% 가중).
- 수동 기록(`record`)은 자동 체크가 전혀 없는 과거 기간에도 일별 바·인시던트·가동률에
  그대로 반영되므로, 뉴스/공지 기반으로 과거 결항을 백필할 수 있습니다.
  2026년 7월 장마 결항(7/9~7/24 동부 구간 결항 + 7/23~24 전면 중단)이 이런 방식으로
  `data/overrides.jsonl` 에 기록돼 있습니다.
